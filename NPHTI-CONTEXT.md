...
- **Angled photo frame** — an angled `clip-path` frame paired with an offset color-block "backing" shape instead of a plain rounded-rect + border, used wherever a hero photo needs to feel native to the palette rather than bolted on (homepage, Mid-Year Meetup, Contact). **Retired: the duotone SVG filter** (`feColorMatrix` to grayscale + `feComponentTransfer` remapping shadows→purple, highlights→aqua, applied via `filter:url(#duotone-hero)`) that used to accompany this frame. It was tried on the homepage hero and several later photos, but real client-supplied photos kept coming back looking flattened/washed-out or muddy under it (see round-by-round notes below), and every hero photo across the site is now shown in full color instead — including the homepage's, reverted in a later round. Don't reintroduce the duotone filter on new photos without checking with the client first.
...
**What stays in the HTML:** everything from the hero down. The homepage's dark "Registration is open" promo banner is page content (not chrome) and stays in `nphti-home.html` only.
...
- **"Scholarships Available" callout** — removed from the Annual Workshops page (`nphti-annual-workshops.html`) at client request, since the 2026 scholarship application deadline (August 15) has passed by the time this round of edits was made and the callout read as still-open. It was a striped card just above the closing content, linking to the new Scholarship Application page:
  > **Scholarships Available** — "NPHTI offers scholarships to help defray registration costs for trainees, clinicians in resource-limited countries, and early-career professionals with limited financial resources. Applications are due **August 15**." → button "Scholarship Details" → `nphti-scholarship-application.html`
...
- **All pages are now built in Bright Geometric**: the four Training subpages (Mid-Year Meetup, Annual Workshops, Webinars, Training Archive), Training Resources, Contact (hero + native Wix Form, see `CONTACT-FORM-SPEC.md`), Find a Provider (hero + a fully custom map/search/results frontend in the iframe itself, backed by a Velo function reading the existing Wix CMS Collection, see `FIND-A-PROVIDER-SPEC.md`), and Donate (all five giving methods from the live site: mail a check, Zelle, Venmo, Stripe, share your story). Plus Leadership, About, Faculty, and Home from earlier rounds. Remaining work is the Wix-side integration items below, not new pages.
